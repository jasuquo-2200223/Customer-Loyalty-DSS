...
# FIX 1: Merged the two init_db() definitions into one complete function.
# The second definition was overwriting the first, causing analysis_results
# and audit_logs tables to never be created.
def init_db():
            ...
            <div class="max-w-7xl mx-auto px-6">
                ...
                <div class="grid md:grid-cols-3 gap-12">
                    <div class="space-y-4">
                        ...
                        <p class="text-slate-600 text-sm leading-relaxed">Discover natural groupings based on dynamic purchase cycles, total invoice counts, and cumulative lifetime spends. Automatically normalized through Box-Cox scaling algorithms.</p>
                    </div>
                    <div class="space-y-4">
                        ...
                        <p class="text-slate-600 text-sm leading-relaxed">Analyze past response and discount sensitivity patterns across each unique consumer cluster to design high-margin campaigns with minimal profit leakage.</p>
                    </div>
                    <div class="space-y-4">
                        ...
                        <p class="text-slate-600 text-sm leading-relaxed">Simulate high-impact scenarios in real-time. Forecast net profits, ROI lifts, and customer migrations instantly prior to investing capital.</p>
                    </div>
                </div>
            </div>
        </section>
            ...
            <div class="max-w-7xl mx-auto px-6">
                <div class="grid lg:grid-cols-2 gap-12 items-center">
                    ...
                    <div class="bg-indigo-600 p-12 rounded-[2.5rem] text-white shadow-xl space-y-6">
                        ...
                        <p class="opacity-80 text-sm leading-relaxed">Sign up to gain full cloud analysis, historical data tracking, strategic simulator presets, and localized business action reports.</p>
                        <a href="/register" class="bg-white text-indigo-700 px-8 py-3.5 rounded-full font-black inline-block shadow-md hover:bg-slate-100 transition">Create Free Manager Account</a>
                    </div>
                </div>
            </div>
        </section>
...
<body class="bg-slate-50 transition-colors duration-300">
    ...
    <main class="max-w-6xl mx-auto mt-10 px-6 pb-20">
        ...
        {% if show_history %}
            <div class="flex justify-between items-center mb-8">
                ...
                <button onclick="clearAllHistory()" class="bg-red-50 text-red-600 px-6 py-2.5 rounded-2xl text-xs font-bold hover:bg-red-100 transition shadow-sm">
                    <i class="fas fa-trash mr-2"></i>Wipe History
                </button>
                {% endif %}
            </div>
            {% if history %}
        ...
        {% else %}
            ...
            <div class="bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-sm mb-8">
                ...
                <div id="executiveInsightsList" class="space-y-4"></div>
            </div>
            ...
            <div class="bg-white p-8 rounded-[2.5rem] border shadow-sm mb-12">
                ...
                <!-- FIX 5: Added the missing projectionStatus element that runSimulation() references -->
                <div class="mt-4 text-center">
                    <span class="text-xs text-slate-400 font-bold" id="projectionStatus">Status: Ready</span>
                </div>
                ...
                <div class="mt-6 bg-slate-50 border border-slate-200 p-6 rounded-2xl space-y-3">
                    ...
                    <div id="dssAdviceContainer" class="space-y-2"></div>
                </div>
            </div>
                            ...
                            <div class="grid md:grid-cols-2 gap-10">
                                ...
                                <div class="space-y-6 flex flex-col justify-start">
                                    <div class="p-8 bg-indigo-50 rounded-[2rem] border border-indigo-100 h-full flex flex-col justify-center">
                                        ...
                                        <p class="text-sm text-slate-800 leading-relaxed font-bold" id="interpretation-{{ loop.index0 }}">Analyzing behavioral details...</p>
                                    </div>
                                </div>
                            </div>
                        </div>
    ...
    <script>
        ...
        function interpretChart(idx) {
            ...
            let interpretation = "";
            if (idx === 0) {
                interpretation = "These are your absolute best customers! They shop very frequently, bought from you recently, and spend much more than the average shopper (₦" + (data.M || 0).toLocaleString() + " on average). Since they already love your brand, you do not need to give them heavy discounts to keep them. Instead, treat them like VIPs—provide early access to new arrivals, priority customer support, or invite them to a special loyalty club.";
            } else if (idx === 1) {
                interpretation = "This group is your reliable, steady backbone. They might not buy your most expensive items, but they visit your store regularly and have purchased from you recently. Since their favorite category is " + (data.TopCategory || "General") + ", they will respond wonderfully to friendly product reminders or small cross-sell bundles related to that category. Keep them happy with standard, consistent engagement!";
            } else if (idx === 2) {
                interpretation = "Attention! This is a highly critical group that is currently slipping away. These customers used to be great, high-spending supporters, but they haven't bought anything in quite a long time (" + (data.R || 0) + " days on average) and are at a high risk of leaving for good. You should act quickly to win them back—send them a special 'We Miss You' discount code or a personalized offer in their favorite category before they forget about us completely.";
            } else {
        ...
        function syncInputs(sourceId, targetId) {
            ...
            document.querySelectorAll('.scenario-btn').forEach(btn => btn.classList.remove('active-preset', 'bg-indigo-600', 'text-white'));
        }
        ...
        function runSimulation() {
            ...
            document.getElementById('projectedRevenue').innerText = "₦" + netProfit.toLocaleString();
            document.getElementById('projectedRoi').innerText = projectedRoi + "%";
            ...
            adviceContainer.innerHTML =
                '<p class="text-sm text-slate-800 leading-normal border-b border-slate-200/80 pb-3"><span class="text-indigo-600 font-bold"><i class="fas fa-users mr-2"></i>Strategy Option A (Increase Customer Turnout):</span> If we get just <strong>5 out of 100 more customers</strong> to respond to our campaigns, we can generate an additional <strong class="text-slate-950">₦' + optUpgradeVal.toLocaleString() + '</strong> in gross revenue.</p>' +
                '<p class="text-sm text-slate-800 leading-normal border-b border-slate-200/80 py-3"><span class="text-indigo-600 font-bold"><i class="fas fa-shopping-basket mr-2"></i>Strategy Option B (Increase Basket Value):</span> If we can encourage each customer to add just one more accessory (worth <strong>₦1,000</strong>) to their order, we can make an extra <strong class="text-slate-950">₦' + optSpendVal.toLocaleString() + '</strong>.</p>' +
                '<p class="text-sm text-slate-800 leading-normal pt-3"><span class="text-indigo-600 font-bold"><i class="fas fa-lightbulb mr-2"></i>Strategic Recommendation Guide:</span> ' + leverageAdvice + '</p>';

            // FIX 5: Safe access to projectionStatus element
            const statusEl = document.getElementById('projectionStatus');
            if (statusEl) statusEl.innerText = "Status: Projecting " + expectedUpgrades.toLocaleString() + " responsive customers";
        }
        ...
        function populateExecutiveInsights() {
            ...
            const atRisk = analysisData[2] || { Label: "At-Risk Customers", R: 180 };
            const coldChurn = analysisData[3] || { Label: "Cold Churn", R: 250 };
            listElement.innerHTML =
                '<div class="flex flex-col md:flex-row md:items-center justify-between p-5 bg-emerald-50/60 border border-emerald-100 rounded-2xl space-y-2 md:space-y-0">' +
                    '<div class="flex items-center space-x-3"><div class="w-8 h-8 rounded-full bg-emerald-100 text-emerald-700 flex items-center justify-center font-black text-sm">1</div>' +
                    '<div><p class="text-[10px] text-slate-400 font-bold uppercase tracking-wider leading-none mb-1">Most Valuable Segment</p>' +
                    '<p class="text-base font-black text-slate-800">' + champions.Label + ' <span class="text-slate-400 font-medium">→</span> Highest revenue contributors</p></div></div>' +
                    '<div class="flex items-center space-x-4"><span class="px-3 py-1 bg-emerald-100 text-emerald-700 text-[10px] font-black rounded-full uppercase tracking-wider">Priority: Retain & Upsell</span>' +
                    '<span class="text-xs font-bold text-slate-600">Immediate Action: VIP support & premium bundles</span></div></div>' +
                ...
                '<div class="flex flex-col md:flex-row md:items-center justify-between p-5 mt-4 bg-amber-50/60 border border-amber-100 rounded-2xl space-y-2 md:space-y-0">' +
                    '<div class="flex items-center space-x-3"><div class="w-8 h-8 rounded-full bg-amber-100 text-amber-700 flex items-center justify-center font-black text-sm">2</div>' +
                    '<div><p class="text-[10px] text-slate-400 font-bold uppercase tracking-wider leading-none mb-1">Strong Recovery Opportunity</p>' +
                    '<p class="text-base font-black text-slate-800">' + atRisk.Label + ' <span class="text-slate-400 font-medium">→</span> High Churn Risk (Ref: ' + atRisk.R + ' days idle)</p></div></div>' +
                    '<div class="flex items-center space-x-4"><span class="px-3 py-1 bg-amber-100 text-amber-700 text-[10px] font-black rounded-full uppercase tracking-wider">Priority: FIRST / CRITICAL</span>' +
                    '<span class="text-xs font-bold text-slate-600">Immediate Action: Deploy category-specific discounts</span></div></div>' +
                ...
                '<div class="flex flex-col md:flex-row md:items-center justify-between p-5 mt-4 bg-rose-50/60 border border-rose-100 rounded-2xl space-y-2 md:space-y-0">' +
                    '<div class="flex items-center space-x-3"><div class="w-8 h-8 rounded-full bg-rose-100 text-rose-700 flex items-center justify-center font-black text-sm">3</div>' +
                    '<div><p class="text-[10px] text-slate-400 font-bold uppercase tracking-wider leading-none mb-1">Inactive customer base</p>' +
                    '<p class="text-base font-black text-slate-800">' + coldChurn.Label + ' <span class="text-slate-400 font-medium">→</span> High Churn Risk (Ref: ' + coldChurn.R + ' days idle)</p></div></div>' +
                    '<div class="flex items-center space-x-4"><span class="px-3 py-1 bg-rose-100 text-rose-700 text-[10px] font-black rounded-full uppercase tracking-wider">Priority: Low-Cost Reactivation</span>' +
                    '<span class="text-xs font-bold text-slate-600">Immediate Action: Seasonal clearances & clearance emails</span></div></div>';
        }
        ...
        function triggerConfetti() {
            ...
            (function frame() {
                confetti({ particleCount: 4, angle: 60, spread: 60, origin: { x: 0, y: 0.8 } });
                confetti({ particleCount: 4, angle: 120, spread: 60, origin: { x: 1, y: 0.8 } });
...
def run_dss_engine(df):
    ...
    for standard_col, options in col_mapping.items():
        ...
        for option in options:
            if option.lower() in lower_cols:
                idx = lower_cols.index(option.lower())
    ...
    summ = rfm.groupby('Cluster').agg({
        ...
        'AvgDiscount': 'mean', 'TotalQuantity': 'mean'
    }).reset_index()
...
def analyze():
    ...
    try:
        ...
        # Segment labels, tips, and brief descriptions
        segment_config = [
            {
                'Label': 'Elite Champions',
                'Priority': 'High',
                'Action': 'Exclusive early-access rewards and VIP club invitations.',
                'Essence': 'Top-tier customers: High frequency, high spend, very recent.',
                # FIX 4: Added BriefTip key that the template references
                'BriefTip': 'Invite to VIP programme & offer early product access.',
                'RecencyText': 'Very Recent',
                'FrequencyText': 'Very Often',
            },
            {
                'Label': 'Loyal Sustainers',
                'Priority': 'Normal',
                'Action': 'Cross-sell related accessories to increase basket size.',
                'Essence': 'Steady, reliable shoppers forming your revenue backbone.',
                'BriefTip': 'Bundle accessories & send category newsletters.',
                'RecencyText': 'Recent',
                'FrequencyText': 'Regular',
            },
            {
                'Label': 'At-Risk Customers',
                'Priority': 'High',
                'Action': 'Deploy personalized recovery discount vouchers.',
                'Essence': 'Formerly high-value, now showing signs of drifting.',
                'BriefTip': 'Send "We Miss You" voucher with category discount.',
                'RecencyText': 'Fading',
                'FrequencyText': 'Slowing',
            },
            {
                'Label': 'Cold Churn',
                'Priority': 'CRITICAL',
                'Action': 'Target with seasonal clearance campaigns.',
                'Essence': 'Inactive for an extended period; needs low-cost reactivation.',
                'BriefTip': 'Include in low-cost seasonal clearance blasts only.',
                'RecencyText': 'Very Old',
                'FrequencyText': 'Rare',
            },
        ]
        ...
        for item in results:
            ...
            cfg = segment_config[cluster_id] if cluster_id < len(segment_config) else segment_config[-1]
            item.update(cfg)
            item['R'] = round(item['Recency'], 0)
            item['F'] = round(item['Frequency'], 0)
            item['M'] = round(item['Monetary'], 0)
            item['AvgDiscount'] = round(item.get('AvgDiscount', 0), 1)
...
def save_result():
    ...
    conn = get_db_connection()
    conn.execute(
        'INSERT INTO analysis_results (username, rfm_json, summary_json, base_avg_json, best_k) VALUES (?, ?, ?, ?, ?)',
        (session['user'], json.dumps({}), json.dumps(user_cache['results']), json.dumps(user_cache['base_avg']), 4)
    )
    conn.commit()
    conn.close()
...
def home():
    ...
    return render_template_string(
        DASHBOARD_HTML, results=results,
        theme=user['theme'] if user else 'light',
        show_save_prompt=session.get('show_prompt'),
        base_avg=base_avg, kpis=kpis, silhouette=silhouette,
        show_confetti=show_confetti, show_history=False, history=None
    )


# FIX 2: Fixed the login route — the redirect and flash were outside the if-block
# due to wrong indentation, so login never actually authenticated anyone.
    ...
    return render_template_string("""
        <!DOCTYPE html><html>""" + HTML_HEAD + """<body class="bg-slate-900 flex items-center justify-center min-h-screen">
            <div class="bg-white p-12 rounded-[3rem] shadow-2xl w-full max-w-md text-center border border-slate-200">
                ...
                <div class="mt-8 flex justify-center text-xs font-black text-indigo-600 tracking-widest uppercase">
...
# FIX 3: Fixed register route — on success it now redirects to /login
# instead of falling through and re-rendering the empty form.
...
def register():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            conn.commit()
            conn.close()
            log_audit_action(u, "New manager account registered.")
...
def view_history(id):
    ...
    if res:
        ...
        kpis = {"total_cust": len(results) * 1250}
        temp_analysis_cache[session['user']] = {
            'results': results, 'base_avg': base_avg,
            'kpis': kpis, 'silhouette': "0.376"
        }
        session['show_prompt'] = False
        log_audit_action(session['user'], f"Restored historical analytical session ID {id}.")
        return redirect('/dashboard')
    return redirect('/history')
...
def delete_history(id):
    ...
    conn.commit()
    conn.close()
    log_audit_action(session['user'], f"Deleted saved analysis log ID {id}.")
...
def clear_all_history():
    ...
    conn.commit()
    conn.close()
    log_audit_action(session['user'], "Cleared all historical records.")
...
def delete_account():
    ...
    conn.commit()
    conn.close()
    log_audit_action(session.get('user', 'anonymous'), "Deleted account.")
    session.clear()
...
def reset_analysis():
    ...
    session['show_prompt'] = False
    log_audit_action(session.get('user', 'system'), "Wiped live cache to upload new dataset.")
    return redirect('/dashboard')


@app.route('/logout')
def logout():
    log_audit_action(session.get('user', 'anonymous'), "Logged out.")
    session.clear()
    return redirect('/login')


# FIX 6 & 7: Removed the duplicate __main__ blocks and the bare `os` reference
# that caused a NameError. One clean entry point with os imported at the top.
