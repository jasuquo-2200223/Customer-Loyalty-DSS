...
# --- FLASH MESSAGE HELPER (inline styles to avoid Tailwind CDN purging issues) ---
FLASH_BLOCK = """
{% with msgs = get_flashed_messages(with_categories=true) %}
{% for cat, m in msgs %}
<div style="padding:12px 16px; border-radius:10px; margin-bottom:16px; font-size:13px; font-weight:700;
    background:{% if cat=='error' %}#fef2f2{% else %}#f0fdf4{% endif %};
    color:{% if cat=='error' %}#dc2626{% else %}#16a34a{% endif %};
    border-left:4px solid {% if cat=='error' %}#dc2626{% else %}#16a34a{% endif %};">
    {{ m }}
</div>
{% endfor %}
{% endwith %}
...
<body class="bg-slate-50 transition-colors duration-300">
            ...
            <div class="grid md:grid-cols-2 gap-8">
                ...
                <div onclick="openModal('{{ loop.index0 }}')" class="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-200 cursor-pointer hover:border-indigo-400 transition transform hover:-translate-y-1">
                    <div class="flex justify-between items-start mb-6">
                        ...
                        <div class="text-right">
                            <span class="inline-block px-3 py-1 text-[9px] font-black uppercase rounded-full tracking-wider" style="background:{% if item.Priority == 'CRITICAL' %}#fee2e2{% elif item.Priority == 'High' %}#fef3c7{% else %}#f1f5f9{% endif %}; color:{% if item.Priority == 'CRITICAL' %}#dc2626{% elif item.Priority == 'High' %}#b45309{% else %}#475569{% endif %};">
    ...
    <script>
        ...
        function runSimulation() {
            ...
            let leverageAdvice = accessAov > (targetUpgrade * 120)
                ? "Our analytical model recommends focusing primarily on <strong>Strategy Option A (Customer Turnout)</strong>. Getting more people to respond to our campaign is far easier and will bring in significantly more net profit than trying to force customers to spend more per order. Focus your budget on better marketing emails and personalized discount subject lines."
                : "Our analytical model recommends focusing primarily on <strong>Strategy Option B (Basket Value)</strong>. Encouraging already-interested customers to spend slightly more yields superior financial returns. We recommend focusing on product bundling, category-based cross-selling (e.g., offering accessory add-ons), or setting free-shipping thresholds.";
        ...
        function populateExecutiveInsights() {
            ...
            '<div class="flex flex-col md:flex-row md:items-center justify-between p-5 bg-emerald-50/60 border border-emerald-100 rounded-2xl space-y-2 md:space-y-0">' +
                '<div class="flex items-center space-x-3">' +
                    ...
                    '<p class="text-base font-black text-slate-800">' + champions.Label + ' <span class="text-slate-400 font-medium">→</span> Highest revenue contributors</p></div>' +
                '</div>' +
                '<div class="flex items-center space-x-4">' +
            ...
            '<div class="flex flex-col md:flex-row md:items-center justify-between p-5 mt-4 bg-amber-50/60 border border-amber-100 rounded-2xl space-y-2 md:space-y-0">' +
                '<div class="flex items-center space-x-3">' +
                    ...
                    '<p class="text-base font-black text-slate-800">' + atRisk.Label + ' <span class="text-slate-400 font-medium">→</span> High Churn Risk (Ref: ' + atRisk.R + ' days idle)</p></div>' +
                '</div>' +
                '<div class="flex items-center space-x-4">' +
            ...
            '<div class="flex flex-col md:flex-row md:items-center justify-between p-5 mt-4 bg-rose-50/60 border border-rose-100 rounded-2xl space-y-2 md:space-y-0">' +
                '<div class="flex items-center space-x-3">' +
                    ...
                    '<p class="text-base font-black text-slate-800">' + coldChurn.Label + ' <span class="text-slate-400 font-medium">→</span> High Churn Risk (Ref: ' + coldChurn.R + ' days idle)</p></div>' +
                '</div>' +
                '<div class="flex items-center space-x-4">' +
    ...
    try:
        ...
        for encoding in ('utf-8', 'latin-1', 'cp1252', 'utf-8-sig'):
            try:
                ...
                continue
        if df is None:
            flash("Could not read the CSV file. Please ensure it is UTF-8 or standard Excel encoding.", "error")
            return redirect('/dashboard')
        ...
        for item in results:
            ...
            if item['Cluster'] == 0:
                ...
                item['BriefTip'] = "VIP rewards & early product access"
            elif item['Cluster'] == 1:
                ...
                item['BriefTip'] = "Cross-sell bundles in favourite category"
            elif item['Cluster'] == 2:
                ...
                item['BriefTip'] = "Personalised 'We Miss You' voucher"
            else:
                ...
                item['BriefTip'] = "Low-cost seasonal clearance emails"

        temp_analysis_cache[session['user']] = {
            'results': results, 'base_avg': base_avg,
            'kpis': {'total_cust': total_cust}, 'silhouette': f"{sil:.3f}"
        }
...
def save_result():
    ...
    if not user_cache:
        ...
        return redirect('/dashboard')
    conn = get_db_connection()
    conn.execute('''INSERT INTO analysis_results (username, rfm_json, summary_json, base_avg_json, best_k)
                    VALUES (?, ?, ?, ?, ?)''',
                 (session['user'], json.dumps({}), json.dumps(user_cache['results']),
                  json.dumps(user_cache['base_avg']), 4))
...
def home():
    ...
    return render_template_string(DASHBOARD_HTML, results=results, theme=user['theme'] if user else 'light',
                                  show_save_prompt=session.get('show_prompt'), base_avg=base_avg, kpis=kpis,
                                  silhouette=silhouette, show_confetti=show_confetti, show_history=False, history=None)
...
def forgot():
    if request.method == 'POST':
        ...
        flash("If your email exists in our system, a reset link has been sent.", "success")
        return redirect('/login')
    return render_template_string("""
        <!DOCTYPE html><html>""" + HTML_HEAD + """<body class="bg-slate-900 flex items-center justify-center min-h-screen">
...
def register():
    # Always initialize with empty strings to prevent Jinja2 UndefinedError on GET requests
    form = {'f': '', 'l': '', 'u': '', 'e': ''}
    if request.method == 'POST':
        f = request.form.get('f', '').strip()
        l = request.form.get('l', '').strip()
        ...
        if not f or not l or not u or not e or not p:
        ...
        try:
            ...
            mail_sent = send_mail(
                e,
                "Verify Your Customer Loyalty DSS Account",
                f"Welcome to Customer Loyalty DSS!\n\nClick the link below to activate your account:\n\n{link}\n\nThis link expires in 1 hour.\n\nIf you did not register, please ignore this email."
            )

            if mail_sent:
                flash("Registration successful! Please check your inbox for a verification link.", "success")
                return redirect('/login')
            else:
                # SMTP blocked (common on free hosting) — show inline verification button
                return f"""<!DOCTYPE html><html>
                <head><meta charset="UTF-8"><title>Verify Account</title>
                <script src="https://cdn.tailwindcss.com"></script>
                <link href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css" rel="stylesheet">
                </head>
                <body class="bg-slate-900 flex items-center justify-center min-h-screen">
                    <div class="bg-white p-12 rounded-[3rem] shadow-2xl text-center max-w-md w-full border border-slate-200">
                        <div class="w-20 h-20 bg-emerald-50 rounded-3xl flex items-center justify-center mx-auto mb-6 text-emerald-500 text-4xl">
                            <i class="fas fa-envelope-open-text"></i>
                        </div>
                        <h2 class="text-2xl font-black text-slate-800 mb-2">Almost There!</h2>
                        <p class="text-slate-500 text-sm mb-2">Account created for <strong>{e}</strong>.</p>
                        <p class="text-slate-400 text-xs mb-8">Email delivery is unavailable on this server. Click below to manually verify and activate your account:</p>
                        <a href="/verify/{token}" class="bg-indigo-600 text-white px-8 py-4 rounded-2xl font-black inline-block hover:bg-indigo-700 transition shadow-lg">
                            <i class="fas fa-check-circle mr-2"></i>Verify & Activate Account
                        </a>
                        <p class="mt-8 text-xs text-slate-400"><a href="/login" class="hover:underline text-indigo-400">Back to Login</a></p>
                    </div>
                </body></html>"""
...
def verify(token):
    try:
        ...
        flash("This verification link has expired or is invalid. Please register again.", "error")
...
def history():
    ...
    user = conn.execute("SELECT theme FROM users WHERE username=?", (session['user'],)).fetchone()
    conn.close()
    return render_template_string(DASHBOARD_HTML, history=rows, show_history=True, theme=user['theme'],
                                  results=None, base_avg=None, kpis=None, silhouette=None, show_confetti=False)

@app.route('/view_history/<int:id>')
def view_history(id):
    conn = get_db_connection()
    res = conn.execute("SELECT summary_json, base_avg_json, timestamp FROM analysis_results WHERE id=? AND username=?",
    ...
    if res:
        ...
        temp_analysis_cache[session['user']] = {
            'results': results, 'base_avg': base_avg, 'kpis': kpis, 'silhouette': "0.376"
        }
